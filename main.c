...
typedef enum  {
  META_COMMAND_SUCCES, // default value 0
  META_COMMAND_UNRECOGNIZED_COMMAND // default value 1
} MetaCommandResult;
...
PrepareResult prepare_insert(InputBuffer* input_buffer, Statement* statement) {
  ...
  if (id < 0) {
    printf("Please use positive intgets for id\n");
...
const u_int32_t USERNAME_OFFEST = ID_OFFSET + ID_SIZE;
const u_int32_t EMAIL_OFFSET = USERNAME_OFFEST + USERNAME_SIZE;
...
void deserialize_row(void* source, Row* destination)  {
  ...
  memcpy(&(destination->username), source + USERNAME_OFFEST, USERNAME_SIZE);
